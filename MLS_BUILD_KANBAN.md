...
| 9 | Take Lifecycle Sorting, Sidecars, and Manifest Rollup | AFK | Core lifecycle routing landed in `6d8340a`; operator review override landed in `937e427`. Closed takes now get explicit `takeClassification` and `classificationReason`; critical REC consensus flags route to `qc_fail/`; false-roll flags route to `false_roll/`; warning-only issues such as SDI dropout remain in `takes/` with QC metadata. Sidecar filenames follow the final folder name, clip-level QC flags are populated, manifest rollup scans all sort folders, and QC-fail/false-roll takes create a non-blocking Operator Console review item. Review can keep the automatic classification or move the take between `takes/`, `qc_fail/`, and `false_roll/` with an operator override flag. `swift build` passed. | App/hardware validation: record 9/9, 8/9, 1/9, middle partial, late-stop, and false-roll patterns; inspect folder placement, per-take `data.json`, `manifest.json`, and Review Take override behavior. |
